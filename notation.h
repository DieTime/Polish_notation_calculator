...
void parse_expression(std::string& expression)
{
	...
	for (int i = 0; i <= number_of_operators_for_parsing; i++)
	{
		...
		if (!positions.empty())
		{
			...
			for (int j : positions)
			{
				expression.replace(j + corrective, list_of_operators[i].length(), " " + list_of_operators[i] + " ");
				corrective += 2;
			}
		}
	}
...
int correct_expression(std::string& expression)
{
	...
	if (is_operator(convert_to_string(expression[0])) && (expression[0] != '-'))
		return 0;

	// Convert expression to correct form
	parse_expression(expression);

	/*If the word found is not a number
	  or operator, it returns 0 */
